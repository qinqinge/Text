...
{
    static NSDateFormatter *_xmhqDateFormatter = nil;
    static dispatch_once_t onceToken;
    
    XMHQDispatchOnce(onceToken, ^{
        
        if (!_xmhqDateFormatter) {
            _xmhqDateFormatter = [[NSDateFormatter alloc]init];
        }
    });
    return _xmhqDateFormatter;
}

+ (NSString *)detailTimeAgoString:(NSDate *)date
{
    if (XMHQCheckObjectNull(date)) {
        return nil;
    }
    long long timeNow = [date timeIntervalSince1970];
    NSCalendar * calendar=[XMHQDateUitil sharedCalendar];
...
{
    return [XMHQDateUitil detailTimeAgoString:XMHQDateFromTimeInterval(timeInterval)];
}
...
+ (NSString *)stringDaysAgoAgainstMidnight:(BOOL)flag withDate:(NSDate *)date {
    ...
    NSUInteger daysAgo = (flag) ? [XMHQDateUitil daysAgoAgainstMidnight:date] : [XMHQDateUitil daysAgoFromNow:date];
    NSString *text = nil;
    switch (daysAgo) {
        case 0:
            text = @"今天";
...
+ (NSString *)stringDaysAgo:(NSDate *)date
{
    ...
    return [XMHQDateUitil stringDaysAgoAgainstMidnight:YES withDate:date];
}
...
+ (NSString *)weekDayString:(NSDate *)date
{
    ...
    NSString *weekName = [weekNameDict objectForKey:@(XMHQDateGetWeekDay(date))];
...
+ (NSString *)weekNumberString:(NSDate *)date
{
    ...
    return [NSString stringWithFormat:@"第%lu周",XMHQDateGetWeekNum(date)];
}
...
+ (NSDate *)dateFromString:(NSString *)string
{
    ...
    return [XMHQDateUitil dateFromString:string withFormat:kNSDateHelperFormatSQLDate];
}
...
+ (NSDate *)dateTimeFromString:(NSString *)string
{
    ...
    return [XMHQDateUitil dateFromString:string withFormat:kNSDateHelperFormatSQLDateWithTime];
}

+ (NSDate *)dateFromString:(NSString *)string withFormat:(NSString *)format
{
    if (XMHQCheckKeyValueHasNull(string, format)) {
        return nil;
    }
...
{
    if (XMHQCheckKeyValueHasNull(string, date)) {
        return nil;
    }
...
+ (NSString *)stringFromDate:(NSDate *)date
{
    ...
    return [XMHQDateUitil stringFromDate:date withFormat:kNSDateHelperFormatSQLDateWithTime];
}
...
{
    return [XMHQDateUitil birthdayToAge:XMHQDateFromTimeInterval(date)];
}

+ (NSString *)dateToConstellation:(NSDate *)date
{
    NSInteger day = [XMHQDateUitil day:date];
    NSInteger month = [XMHQDateUitil month:date];
...
{
    return [XMHQDateUitil dateToConstellation:XMHQDateFromTimeInterval(date)];
}
